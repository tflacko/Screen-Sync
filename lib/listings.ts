...
import { LISTING_FEE_USDC } from './constants';
import { getConnection, getTreasury } from './connection';
import { usdcReceived } from './bookings';
...
export async function getOnChainListings(): Promise<Listing[]> {
  ...
  try {
    ...
    for (const tx of txs) {
      ...
      // Anti-spam: the USDC registry fee must actually have been paid.
      if (LISTING_FEE_USDC > 0 && usdcReceived(tx, treasuryStr) + 1e-6 < LISTING_FEE_USDC) continue;
